...
exports.CPMEngine = void 0;
class CPMEngine {
    // =========================
    // 🔍 CYCLE DETECTION
    // =========================
    static detectCycles(activities) {
        const graph = new Map();
    ...
    // 🔢 TOPO SORT
    // =========================
    static topologicalSort(activities) {
    ...
    // ▶ FORWARD PASS
    // =========================
    static forwardPass(nodes, order) {
        const map = new Map(nodes.map(n => [n.id, n]));
        order.forEach(id => {
            const n = map.get(id);
            n.es = Math.max(0, ...(n.predecessors || []).map(p => map.get(p)?.ef || 0));
    ...
    // ◀ BACKWARD PASS
    // =========================
    static backwardPass(nodes, order, duration) {
        const map = new Map(nodes.map(n => [n.id, n]));
        const succ = new Map();
    ...
    // =========================
    static findAllCriticalPaths(nodes, projectDuration) {
        const paths = [];
        function dfs(curr, path) {
            ...
            if (curr.ef === projectDuration) {
                ...
                for (const next of nodes) {
                    if (next.predecessors.includes(curr.id) &&
                        next.isCritical &&
                        next.es === curr.ef) {
                        dfs(next, path);
                    }
    ...
    // 🚀 MAIN CPM
    // =========================
    static calculate(activities, overrideDurations) {
...
exports.CPMEngine = CPMEngine;
//# sourceMappingURL=cpmEngine.js.map